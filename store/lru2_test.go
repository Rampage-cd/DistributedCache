...
//为什么要定义一个新的string类型呢？为了能够绑定Len方法从而实现Value接口
...
func TestCacheBasic(t *testing.T){
	//启动多个自测试，拆分不同的场景(无论测试内部还是测试外部的代码，后面的逻辑都是建立在前面测试通过了的基础之上)
	//1.初始化测试
	...
	t.Run("添加和获取", func(t *testing.T) {
		...
		// 更新现有项
		status = c.put("key1", testValue("value2"), 200, onEvicted)
		if status != 0 {
		...
		// 验证更新后的值
		node, _ = c.get("key1")
		if node.v.(testValue) != "value2" || node.expireAt != 200 {
			t.Fatalf("更新项后值不一致: %+v", *node)//%+v用于显示节点的字段名和字段值
		}
	})
	...
	t.Run("删除操作", func(t *testing.T) {
		...
		}//由于删除操作并不更改哈希表（只有在容量满后，节点复用时才会更改），故get函数应该返回1才算正常
		if node.expireAt != 0 {
	...
	t.Run("容量和淘汰", func(t *testing.T) {
		...
		}//rune是int32的别名，rune('0'+i)用于将0-9转化为'0'-'9'
	...
	t.Run("遍历缓存",func(t *testing.T){
		...
		//应该有三个键
		if len(keys) != 3{
			t.Fatalf("应该有3个键，实际有%d个",len(keys))
		}

		//键应该是反向添加顺序
		expectedKeys := []string{"key3","key2","key1"}
		for i,key := range expectedKeys{
			if i>=len(keys) || key!=keys[i]{
				t.Fatalf("第%d个键应该为%s，实际为%s",i,key,keys[i])
			}
		}
		...
		c.walk(func(key string,value Value,expireAt int64) bool{
			...
			return len(earlyKeys) < 2//只收集前两个键
		})

		//应该只有两个键
		if len(earlyKeys) != 2{
			t.Fatalf("因该有2个键，实际有%d个",len(earlyKeys))
		}
	})
}
...
func TestLRU2StoreBasicOperations(t *testing.T){
	...
	// 测试更新
	err = store.Set("key1", testValue("value1-updated"))//更新与添加操作并不涉及缓存项目从一级到二级
	if err != nil {
		t.Errorf("Set update failed: %v", err)
	}//此时一级和二级缓存中应该各有一个key1，且值不相同，如果后续Get，返回的是一级缓存中的key1，同时二级缓存中的key1值将更改为一级缓存中的key1值（从而实现统一）
...
//2.测试LRU2Store的LRU替换策略（理想中一级缓存满了，淘汰项应淘汰到二级缓存中，二级缓存也满了，才会丢弃）
...
func TestLRU2StoreHitRatio(t *testing.T) {
	...
	if hitRatio < 0.25 || hitRatio > 0.45 {
