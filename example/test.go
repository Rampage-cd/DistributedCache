...
import (
	...
	lcache "github.com/Rampage-cd/DistributedCache"
	//lcache是导入包时起的别名
)

func main(){
	//添加命令行参数，用于区分不同节点(参数名，默认值，说明)
	port := flag.Int("port",8001,"节点端口")
	nodeID := flag.String("node","A","节点标识符")//port和nodeID都是指针类型
	flag.Parse()
	...
	}//picker是ClientPicker结构体类型的,实现了PeerPicker接口
